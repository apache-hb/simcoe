...
Image Font::drawText(std::u32string_view text) {
    ...
    for (size_t i = 0; i < text.size(); i++) {
        ...
        if (FT_Error error = FT_Load_Char(face, text[i], FT_LOAD_RENDER)) {
            logAssert("failed to load glyph (index={}, glyph={}, fterr={})", i, uint32_t(text[i]), FT_Error_String(error));
        }
